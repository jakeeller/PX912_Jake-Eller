program main
    ...
    REAL(REAL64) :: h, G, nu, W, flux_ana, flux_num, pi ! Mesh step
...
!---------------! Identify nodal positions r ( i ) and create analytic solution f_ana
    DO i =1 , n
        r(i)=0+(i-1)*h
        f_ana(i) = G/(4.0_REAL64*nu)*(W**2 - r(i)**2) ! Calc analytical solution
    END DO
   ...
   !A (1 ,2) =A(2,1)            
    ...
    open (10 , file = "ana.txt" , form = 'formatted')
...
!-------------------------------------------------------------------------------------------
    flux_num=0.0_REAL64
    flux_ana=(pi*(W**4)*G)/(8*nu)!calculate flux analytically
    DO i=1, (N-1)
